...
export const validatePrompt = (prompt: string): boolean => {
  if (!prompt || typeof prompt !== 'string') {
    console.error('Invalid prompt: Expected a string');
    return false;
  }
  
  const trimmed = prompt.trim();
  if (trimmed.length === 0 || trimmed.length > 2000) {
    console.error('Invalid prompt length');
    return false;
  }
  ...
  if (maliciousPatterns.some(pattern => pattern.test(trimmed))) {
    console.error('Malicious pattern detected in prompt');
    return false;
  }
...
class EnhancedRateLimiter {
  ...
  isAllowed(key: string, userId?: string): boolean {
    ...
    if (userId && currentEntry.count >= this.maxRequestsPerUser) {
      console.error('Per-user rate limit exceeded');
      return false;
    }
